...
**当前状态：0.1.0 开发预览。** 已通过核心自动化测试、真实 Codex 程序的无登录隔离协议测试与演示界面检查。真实账号之间的浏览器登录、额度读取、桌面切换与恢复仍需要用户本机验收。不能将“认证文件已替换”当作“桌面 App 已登录成功”。

## 功能

- 原生双栏账号窗口与菜单栏入口，支持搜索、备注和系统深浅色。
...
4. 暂停桌面中的任务后，选择新账号并点击“切换并重开 App”。
5. 在重新打开的桌面 App 中核对账号，再点击工具中的“我已在桌面 App 核对账号”；失败时使用“恢复上次认证”。
...
请先退出已经运行的工具，再启动演示模式。演示模式不初始化账号库、Keychain 或 RPC；所有邮箱都是 `example.com` 示例，所有账号操作禁用。**不要用真实账号窗口截图提交到仓库。**

详见 [安全与数据说明](SECURITY.md)、[验收清单](docs/VALIDATION.md) 和 [设计说明](docs/DESIGN.md)。
