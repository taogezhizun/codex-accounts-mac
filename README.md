...
- **掌握额度** — 默认每 5 分钟自动刷新，分别查看短周期与每周剩余额度。
...
Apple Silicon 已完成本地构建与演示检查；Intel 已构建，尚待实机验收。43 项本地测试及隔离更新测试通过。真实账号登录、自动额度刷新、桌面切换与恢复仍需本机验收；不将“认证文件已替换”视为“桌面登录已验证”。详见[验收清单](docs/VALIDATION.md)。
