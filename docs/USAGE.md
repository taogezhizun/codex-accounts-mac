...
额度查询使用隔离辅助进程和 externally managed token 模式，不在第二个进程中刷新保存的 refresh token。额度查询只读取当前 Codex 登录文件并核对账号身份，不读取钥匙串，也不在刷新后读取恢复备份。0.3.2 起，启动、打开窗口和额度刷新完全不读取钥匙串，也不会要求先检查恢复记录。确认切换后才读取恢复记录；若发现上次切换待核对，或授权被取消，会在退出桌面 App 前停止。重启工具后仍可从「更多操作 → 恢复上次认证…」主动读取备份，系统可能请求授权。工具不会发送预热消息、消费额度重置券或进行自动轮换。
